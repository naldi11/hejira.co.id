...
return new class extends Migration
{
    ...
    {
        Schema::create('jihans_products', function (Blueprint $table) {
            $table->id();
            $table->string('code', 20)->unique();
            $table->string('barcode', 50)->unique()->nullable();
            $table->string('name', 200);
            $table->foreignId('category_id')->constrained('jihans_product_categories');
            $table->foreignId('unit_id')->constrained('jihans_units');
            $table->foreignId('brand_id')->nullable()->constrained('jihans_brands')->nullOnDelete();
    ...
    {
        Schema::dropIfExists('jihans_products');
    }
};
