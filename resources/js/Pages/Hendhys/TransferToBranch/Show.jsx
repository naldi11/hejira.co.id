...
export default function TransferToBranchShow({ transfer: t }) {
    ...
    const route    = window.route;
    return (
        <HendhysLayout pageTitle={`Detail Distribusi ${t.transfer_number}`}>
                ...
                <div className="flex justify-between items-center pt-4 print:hidden">
                    ...
                    <div className="flex gap-3">
                        ...
                        {isCabang && t.status === 'sent' && (
                            <Link href={route('hendhys.transfer-to-branch.receive-form', t.id)} className="inline-flex items-center gap-2 rounded-xl bg-green-600 hover:bg-green-700 text-white px-6 py-2.5 text-sm font-bold shadow-sm transition-colors">
                                <Icon name="check_circle" className="text-[20px]" /> Konfirmasi Penerimaan
                            </Link>
                        )}
                        {/* Pusat: konfirmasi paksa atas nama cabang */}
                        {isPusat && t.status === 'sent' && (
                            <form method="POST" action={route('hendhys.transfer-to-branch.force-receive', t.id)}
                                onSubmit={(e) => { if (!confirm('Konfirmasi bahwa barang sudah diterima di cabang ' + t.branch + '? Stok cabang akan langsung diperbarui.')) e.preventDefault(); }}
                            >
                                <input type="hidden" name="_token" value={document.querySelector('meta[name=csrf-token]')?.content} />
                                <input type="hidden" name="_method" value="POST" />
                                <button type="submit" className="inline-flex items-center gap-2 rounded-xl bg-green-600 hover:bg-green-700 text-white px-6 py-2.5 text-sm font-bold shadow-sm transition-colors">
                                    <Icon name="done_all" className="text-[20px]" /> Konfirmasi Diterima di Cabang
                                </button>
                            </form>
                        )}
                        <a href={route('hendhys.transfer-to-branch.bast', t.id)} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-5 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 transition-colors dark:hover:bg-white/[0.01] dark:border-gray-800 dark:bg-white/[0.03]">
