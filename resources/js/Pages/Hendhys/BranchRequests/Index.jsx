            ...
            <div className="space-y-6">
                ...
                <div className="overflow-hidden rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-white/[0.03] shadow-theme-xs">
                    ...
                    <div className="custom-scrollbar overflow-x-auto"><table className="w-full text-left text-sm"><thead className="border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-white/[0.02] text-gray-500 dark:text-gray-400"><tr><th className="px-6 py-4 font-medium">No. Request</th><th className="px-6 py-4 font-medium">Tanggal</th><th className="px-6 py-4 font-medium">Cabang</th><th className="px-6 py-4 text-center font-medium">Status</th><th className="px-6 py-4 text-center font-medium">Aksi</th></tr></thead>
                        <tbody className="divide-y divide-gray-100 dark:divide-gray-800">{loading ? <SkeletonTableRows rows={6} columns={5} /> : requests.data.length === 0 ? <EmptyState colSpan={5} icon="move_to_inbox" message="Belum ada request." /> : requests.data.map((r) => (<tr key={r.id} className="hover:bg-gray-50 dark:hover:bg-white/[0.01]"><td className="px-6 py-4 font-bold text-gray-800 dark:text-white/90">{r.request_number}</td><td className="px-6 py-4 text-gray-600 dark:text-gray-300">{r.date}</td><td className="px-6 py-4 text-gray-600 dark:text-gray-300">{r.branch ?? 'Gudang Hendhys'}</td><td className="px-6 py-4 text-center"><span className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase ${r.status === 'pending' ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-500/10 dark:text-yellow-400' : r.status === 'completed' ? 'bg-green-100 text-green-700 dark:bg-green-500/10 dark:text-green-400' : 'bg-blue-100 text-blue-700 dark:bg-blue-500/10 dark:text-blue-400'}`}>{r.status}</span></td><td className="px-6 py-4 text-center"><Link href={route('hendhys.branch-requests.show', r.id)} className="text-sm font-medium text-amber-600 dark:text-amber-400 hover:text-amber-800 dark:hover:text-amber-300">Detail</Link></td></tr>))}</tbody></table></div>
