            ...
            <div className="space-y-6">
                ...
                <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
                    ...
                    <div className="custom-scrollbar overflow-x-auto"><table className="w-full text-left text-sm"><thead className="border-b bg-gray-50 text-gray-500 dark:text-gray-400 dark:bg-white/[0.02]"><tr><th className="px-4 py-3">Bulan</th><th className="px-4 py-3 text-center">Transaksi</th><th className="px-4 py-3 text-right">Total</th><th className="px-4 py-3 text-right">Tunai</th><th className="px-4 py-3 text-right">Debit</th><th className="px-4 py-3 text-right">Kredit Card</th><th className="px-4 py-3 text-right">Kredit</th></tr></thead>
                        <tbody className="divide-y">{rows.data?.length === 0 ? <EmptyState colSpan={7} icon="calendar_month" message="Tidak ada data." /> : rows.data?.map((r, i) => (<tr key={i} className="hover:bg-gray-50 dark:hover:bg-white/[0.01]"><td className="px-4 py-3 font-bold">{r.label_bulan}</td><td className="px-4 py-3 text-center font-bold">{r.jumlah_transaksi}</td><td className="px-4 py-3 text-right font-bold">{formatRupiah(r.total_transaksi)}</td><td className="px-4 py-3 text-right">{formatRupiah(r.tunai)}</td><td className="px-4 py-3 text-right">{formatRupiah(r.kartu_debit)}</td><td className="px-4 py-3 text-right">{formatRupiah(r.kartu_kredit)}</td><td className="px-4 py-3 text-right text-red-600 dark:text-red-400">{formatRupiah(r.kredit)}</td></tr>))}</tbody></table></div>
