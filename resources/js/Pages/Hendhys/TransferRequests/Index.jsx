            ...
            <div className="space-y-6">
                {incomingTransfers?.length > 0 && (
                    <div className="rounded-xl border border-green-200 bg-green-50 p-4">
                        <h3 className="mb-2 font-semibold text-green-800"><Icon name="local_shipping" className="mr-1 align-middle text-[20px]" /> Transfer Masuk dari Gudang</h3>
                        <div className="space-y-1">{incomingTransfers.map((t) => (<div key={t.id} className="flex items-center justify-between rounded-lg bg-white p-3 text-sm"><span className="font-medium text-gray-800">{t.transfer_number} — dari {t.creator}</span><span className="text-gray-500">{t.date}</span></div>))}</div>
                    </div>
                )}
                <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
                    <h2 className="text-2xl font-bold tracking-tight text-gray-800">Request Stok ke Gudang</h2>
                                ...
                                {loading ? <SkeletonTableRows rows={6} columns={5} />
                                    ...
                                    : requests.data.map((r) => (
                                        <tr key={r.id} className="hover:bg-gray-50">
                                            <td className="px-6 py-4 font-bold text-gray-800">{r.request_number}</td>
                                            <td className="px-6 py-4 text-gray-600">{r.date}</td>
                                            <td className="px-6 py-4 text-center"><span className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase ${r.status === 'pending' ? 'bg-yellow-100 text-yellow-700' : r.status === 'completed' ? 'bg-green-100 text-green-700' : r.status === 'rejected' ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700'}`}>{r.status}</span></td>
                                            <td className="px-6 py-4 text-gray-500">{r.creator}</td>
                                            <td className="px-6 py-4 text-center"><Link href={route('hendhys.transfer-requests.show', r.id)} className="text-sm font-medium text-amber-600 hover:text-amber-800">Detail</Link></td>
                                        </tr>))}
                            </tbody>
                        </table>
                    </div>
