...
export default function ProductionsForm({ products, units, type = 'aktual', formAction, warning, targetDate, production, isEdit = false }) {
    ...
    const initialItems = production?.details?.map(d => ({
        ...
        unit_id: d.unit_id,
        search_text: ''
    })) || [{ product_id: '', quantity_produced: 1, unit_id: '', search_text: '' }];
    ...
    const title = type === 'prediksi' 
        ? (isEdit ? 'Edit Prediksi Produksi' : 'Buat Prediksi Produksi')
        : 'Catat Produksi Aktual';

    return (
        <HendhysLayout pageTitle={title}>
            <Head title={title} />

            <form onSubmit={submit} className="mx-auto max-w-4xl space-y-6">
                <h2 className="text-2xl font-bold tracking-tight text-gray-800 dark:text-white/90">{title}</h2>
                
                {warning && (
                    <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-amber-800 dark:border-amber-500/20 dark:bg-amber-500/10 dark:text-amber-300">
                    ...
                    <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
                        <div>
                            ...
                            <input 
                                ...
                                required
                                disabled={targetDate !== undefined && targetDate !== null && !isEdit && type === 'aktual'}
                            />
                ...
                <div className="flex justify-end gap-3">
                    ...
                    >
                        {processing ? 'Menyimpan...' : (type === 'prediksi' ? 'Simpan Prediksi' : 'Simpan Aktual & Stok')}
                    </button>
                </div>
            </form>
        </HendhysLayout>
