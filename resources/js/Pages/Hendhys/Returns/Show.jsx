...
export default function ReturnsShow({ return: r }) {
    const handleReceive = () => { 
        if (confirm('Konfirmasi penerimaan retur ini?')) {
            router.post(route('hendhys.returns.receive', r.id)); 
        }
    };

    return (
        <HendhysLayout pageTitle={`Detail Return ${r.return_number}`}>
            ...
            <div className="mx-auto max-w-4xl space-y-6">
                ...
                <div className="flex justify-between items-center pt-4 print:hidden">
                    <Link href={route('hendhys.returns.index')} className="inline-flex items-center gap-2 rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-5 py-2.5 text-sm font-bold text-gray-750 dark:text-gray-300 shadow-theme-xs hover:bg-gray-50 transition-colors dark:hover:bg-white/[0.01] dark:border-gray-800 dark:bg-white/[0.03]">
