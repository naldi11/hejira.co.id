        ...
        <HendhysLayout pageTitle="Kartu Stok Hendhys">
            ...
            <div className="space-y-6">
                <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
                    <div>
                        <h2 className="text-2xl font-bold tracking-tight text-gray-800 dark:text-white/90">Kartu Stok (Pergerakan)</h2>
                        <p className="text-sm text-gray-500 dark:text-gray-400">Riwayat mutasi stok masuk dan keluar</p>
                    </div>
                    <Link href={route('hendhys.stock.index')} className="inline-flex items-center gap-2 rounded-2xl border border-gray-200 bg-white px-5 py-2.5 text-sm font-bold text-gray-700 shadow-theme-xs hover:bg-gray-50 dark:text-gray-300 dark:hover:bg-white/[0.01] dark:border-gray-800 dark:bg-white/[0.03]">
                ...
                <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
                    <div className="border-b border-gray-100 bg-gray-50/50 p-4 dark:border-gray-800 dark:bg-white/[0.01]">
                        <form onSubmit={reload} className="flex flex-wrap items-center gap-3">
                            ...
                            <input type="date" value={form.date_from} onChange={(e) => setForm({ ...form, date_from: e.target.value })} className="rounded-lg border-gray-300 py-2 text-sm focus:border-amber-500 focus:ring-amber-500 dark:border-gray-700 bg-white dark:bg-gray-800 dark:text-white" />
                            <input type="date" value={form.date_to} onChange={(e) => setForm({ ...form, date_to: e.target.value })} className="rounded-lg border-gray-300 py-2 text-sm focus:border-amber-500 focus:ring-amber-500 dark:border-gray-700 bg-white dark:bg-gray-800 dark:text-white" />
                            ...
                            {Object.values(form).some(v => v) && <Link href={route('hendhys.stock.movements')} className="rounded-lg bg-gray-100 px-4 py-2 text-sm font-medium text-red-600 hover:bg-gray-200 dark:text-red-400">Reset</Link>}
                        </form>
                    </div>
                            ...
                            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                                {loading ? <SkeletonTableRows rows={8} columns={6} />
                                    ...
                                    : movements.data.map((m) => (
                                        <tr key={m.id} className="hover:bg-gray-50 dark:hover:bg-white/[0.01]">
                                            <td className="whitespace-nowrap px-6 py-4 text-xs text-gray-500 dark:text-gray-400">{m.created_at}</td>
                                            <td className="px-6 py-4 font-medium text-gray-800 dark:text-white/90">{m.product}</td>
                                            <td className="px-6 py-4 text-center">
                                                <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase ${m.type === 'in' ? 'bg-green-100 text-green-700 dark:bg-green-500/10 dark:text-green-400' : 'bg-red-100 text-red-700 dark:bg-red-500/10 dark:text-red-400'}`}>{m.type === 'in' ? 'Masuk' : 'Keluar'}</span>
                                            </td>
                                            <td className="px-6 py-4 text-center font-black tabular-nums text-gray-800 dark:text-white/90">{m.quantity}</td>
                                            <td className="px-6 py-4"><span className="rounded-lg bg-gray-100 px-2 py-1 text-xs font-medium text-gray-600 dark:text-gray-300">{(m.source ?? '').replace(/_/g, ' ')}</span></td>
                                            <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{m.operator}</td>
                                        </tr>
                                    ))}
                            </tbody>
                        </table>
