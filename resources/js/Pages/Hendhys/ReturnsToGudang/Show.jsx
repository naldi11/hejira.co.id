...
export default function ReturnsToGudangShow({ return: ret }) {
    return (
        <HendhysLayout pageTitle={`Detail Retur ${ret.return_number}`}>
            ...
            <div className="mx-auto max-w-4xl space-y-6">
                <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
                    ...
                    <div className="grid grid-cols-1 gap-6 border-b border-gray-200 p-6 md:grid-cols-3 text-sm dark:border-gray-800">
                        ...
                        <div>
                            ...
                            <p className="text-gray-850 dark:text-gray-300 mt-1 dark:text-white/90">{ret.notes || '-'}</p>
                        </div>
                        {ret.status === 'received' && (
                            <div>
                                ...
                                <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                                    {ret.details?.map((d) => (
                                        <tr key={d.id} className="transition-colors hover:bg-gray-50/50 dark:hover:bg-white/[0.01]">
                                            ...
                                            <td className="px-6 py-4 text-right font-bold text-emerald-600 dark:text-emerald-450">{d.received_quantity !== null ? formatQty(d.received_quantity) : '-'}</td>
                ...
                <div className="flex justify-start pt-4 print:hidden">
                    <Link href={route('hendhys.returns-to-gudang.index')} className="inline-flex items-center gap-2 rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-5 py-2.5 text-sm font-bold text-gray-750 dark:text-gray-300 shadow-theme-xs hover:bg-gray-50 transition-colors dark:hover:bg-white/[0.01] dark:border-gray-800 dark:bg-white/[0.03]">
