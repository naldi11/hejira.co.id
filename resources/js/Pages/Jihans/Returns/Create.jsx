...
export default function JihansReturnsCreate({ products, units }) {
    ...
    const field = 'w-full rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-850 px-3 py-2 text-sm text-gray-800 dark:text-white outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20';
    ...
    return (
        <JihansLayout pageTitle="Buat Retur ke Gudang">
            ...
            <div className="mx-auto max-w-5xl">
                <Link href={route('jihans.returns-to-gudang.index')} className="mb-6 inline-flex items-center gap-1.5 text-sm font-semibold text-orange-600 hover:text-orange-700 dark:text-orange-400 dark:hover:text-orange-300">
                    <Icon name="arrow_back" className="text-[18px]" /> Kembali ke Daftar Retur
                </Link>

                {errors.items && <div className="mb-4 rounded-xl border border-red-200 bg-red-50 dark:border-red-800/50 dark:bg-red-900/20 px-4 py-3 text-sm text-red-700 dark:text-red-400">{errors.items}</div>}
                ...
                <form onSubmit={submit} className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
                    <div className="grid grid-cols-1 gap-6 border-b border-gray-200 bg-orange-500/5 p-6 md:grid-cols-2 dark:border-gray-800">
                        <div>
                            <label className="mb-2 block text-sm font-semibold text-gray-700 dark:text-gray-300">Tanggal Retur <span className="text-red-500">*</span></label>
                            <input type="date" required value={data.date} onChange={(e) => setData('date', e.target.value)} className={field} />
                        </div>
                        <div>
                            <label className="mb-2 block text-sm font-semibold text-gray-700 dark:text-gray-300">Catatan</label>
                    ...
                    <div className="p-6">
                        <div className="mb-4 flex items-center justify-between">
                            <h3 className="font-bold text-gray-850 dark:text-white/90">Barang yang Diretur</h3>
                            <button
                                type="button"
                                onClick={addItem}
                                className="inline-flex items-center gap-1 rounded-lg border border-orange-200 bg-orange-50 px-3 py-1.5 text-sm font-semibold text-orange-700 transition-colors hover:bg-orange-100 dark:border-orange-500/30 dark:bg-orange-500/10 dark:text-orange-400 dark:hover:bg-orange-500/20"
                            >
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-left text-sm">
                                <thead className="border-y border-gray-200 bg-gray-50 text-[10px] font-bold uppercase tracking-wider text-gray-500 dark:border-gray-800 dark:bg-white/[0.02] dark:text-gray-400">
                                    <tr>
                                        <th className="px-3 py-3 font-semibold" style={{ minWidth: 200 }}>Barang <span className="text-red-500">*</span></th>
                                        <th className="px-3 py-3 text-center font-semibold">Stok</th>
                                        <th className="px-3 py-3 font-semibold">Qty <span className="text-red-500">*</span></th>
                                        <th className="px-3 py-3 font-semibold">Kondisi <span className="text-red-500">*</span></th>
                                            ...
                                            <tr key={i} className="hover:bg-gray-50/20 dark:hover:bg-white/[0.005]">
                                                ...
                                                <td className="px-3 py-3 text-center">
                                                    <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${over ? 'bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-400' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300'}`}>
                                                        {formatQty(item.stock)} {item.unit_name}
                                                    </span>
                                                </td>
                                                <td className="px-3 py-3">
                                                    <input
                                                        ...
                                                        className={`${field} ${over ? 'border-red-500 dark:border-red-700 focus:border-red-500 focus:ring-red-500/20' : ''}`}
                                                        placeholder="0"
                                                    />
                                                </td>
                                                <td className="px-3 py-3">
                                                ...
                                                <td className="px-3 py-3 text-center">
                                                    {data.items.length > 1 && (
                                                        <button type="button" onClick={() => removeItem(i)} className="rounded-lg p-1.5 text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10">
                    ...
                    <div className="flex justify-end gap-3 border-t border-gray-200 bg-gray-50/50 p-6 dark:border-gray-800 dark:bg-white/[0.01]">
                        <Link href={route('jihans.returns-to-gudang.index')} className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-5 py-2.5 text-sm font-semibold text-gray-700 dark:text-gray-300 transition-colors hover:bg-gray-50 dark:hover:bg-white/[0.02]">
                            Batal
                        </Link>
                        <button type="submit" disabled={processing} className="flex items-center gap-2 rounded-lg bg-orange-500 px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-orange-600 disabled:opacity-50">
                            <Icon name="local_shipping" className="text-[18px]" /> {processing ? 'Memproses...' : 'Kirim Retur'}
                        </button>
                    </div>
                </form>
            </div>
