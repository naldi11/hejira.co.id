...
export default function ReportBulanan({ rows, filters }) {
    ...
    const reload = (e) => {
        ...
        router.get(route('jihans.reports.bulanan'), p, { preserveState: true });
    };
        ...
        <JihansLayout pageTitle="Laporan Bulanan">
            ...
            <div className="space-y-6">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <h2 className="text-xl font-bold tracking-tight text-gray-800 dark:text-white/90">Laporan Bulanan</h2>
                        <p className="text-sm text-gray-500 dark:text-gray-400">Rekap transaksi dan metode pembayaran bulanan</p>
                    </div>
                    <a
                        href={route('jihans.reports.pdf', 'bulanan') + `?date_from=${form.date_from}&date_to=${form.date_to}`}
                        target="_blank"
                    ...
                    <div className="custom-scrollbar overflow-x-auto">
                        <table className="w-full text-left text-sm">
                            ...
                            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                                {rows.data?.length === 0 ? (
                                    <EmptyState colSpan={7} icon="calendar_month" message="Tidak ada data." />
                                ) : (
                                    rows.data?.map((r, i) => (
                                        <tr key={i} className="transition-colors hover:bg-gray-50/50 dark:hover:bg-white/[0.01]">
                                            <td className="px-6 py-4 text-gray-800 dark:text-white/90 font-medium">{r.label_bulan}</td>
