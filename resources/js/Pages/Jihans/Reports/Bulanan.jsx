...
export default function ReportBulanan({ rows, filters }) {
    ...
    const reload = (e) => { e?.preventDefault(); const p = {}; Object.entries(form).forEach(([k, v]) => { if (v) p[k] = v; }); router.get(route('jihans.reports.bulanan'), p, { preserveState: true }); };
    return (
        <JihansLayout pageTitle="Laporan Bulanan">
            <Head title="Laporan Bulanan" />
            <div className="space-y-6">
                <div className="flex items-center justify-between"><h2 className="text-2xl font-bold text-gray-800">Laporan Bulanan</h2><a href={route('jihans.reports.pdf', 'bulanan') + `?date_from=${form.date_from}&date_to=${form.date_to}`} target="_blank" className="rounded-xl border border-gray-200 bg-white px-5 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Export PDF</a></div>
