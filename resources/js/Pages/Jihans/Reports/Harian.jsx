...
export default function ReportHarian({ rows, filters }) {
    ...
    const reload = (e) => {
        ...
        router.get(route('jihans.reports.harian'), p, { preserveState: true });
    };

    return (
        <JihansLayout pageTitle="Laporan Harian">
            <Head title="Laporan Harian" />
            <div className="space-y-6">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <h2 className="text-xl font-bold tracking-tight text-gray-800 dark:text-white/90">Laporan Harian</h2>
                        <p className="text-sm text-gray-500 dark:text-gray-400">Detail transaksi per pelanggan dan operator kasir</p>
                    </div>
                    <a
                        href={route('jihans.reports.pdf', 'harian') + `?date_from=${form.date_from}&date_to=${form.date_to}`}
                        target="_blank"
                    ...
                    <div className="border-b border-gray-200 bg-gray-50/50 p-5 dark:border-gray-800 dark:bg-white/[0.01]">
                        <form onSubmit={reload} className="flex flex-wrap items-center gap-3">
                            <input
                                ...
                                placeholder="Cari pelanggan/no transaksi..."
                                className="min-w-[200px] flex-1 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-850 px-3 py-2 text-sm text-gray-800 dark:text-white outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20"
                            />
                    ...
                    <div className="custom-scrollbar overflow-x-auto">
                        <table className="w-full text-left text-sm">
                            ...
                            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                                {rows.data?.length === 0 ? (
                                    <EmptyState colSpan={5} icon="receipt_long" message="Tidak ada data." />
                                ) : (
                                    rows.data?.map((r) => (
                                        <tr key={r.id} className="transition-colors hover:bg-gray-50/50 dark:hover:bg-white/[0.01]">
                                            <td className="px-6 py-4 font-mono font-semibold text-gray-800 dark:text-white/90">{r.transaction_number}</td>
                                            <td className="px-6 py-4 text-gray-600 dark:text-gray-300">{r.date}</td>
                                            <td className="px-6 py-4 text-gray-800 dark:text-white/90 font-medium">{r.customer_name}</td>
                                            <td className="px-6 py-4 text-gray-500 dark:text-gray-400">{r.operator}</td>
                                            <td className="px-6 py-4 text-right font-bold text-gray-950 dark:text-white">{formatRupiah(r.grand_total)}</td>
                                        </tr>
                                    ))
                                )}
                            </tbody>
