            ...
            <div className="space-y-6">
                ...
                <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
                    ...
                    <div className="custom-scrollbar overflow-x-auto"><table className="w-full text-left text-sm"><thead className="border-b bg-gray-50 text-gray-500"><tr><th className="px-4 py-3">No. Transaksi</th><th className="px-4 py-3">Tanggal</th><th className="px-4 py-3">Pelanggan</th><th className="px-4 py-3">Operator</th><th className="px-4 py-3 text-right">Total</th></tr></thead>
                        <tbody className="divide-y">{rows.data?.length === 0 ? <EmptyState colSpan={5} icon="receipt_long" message="Tidak ada data." /> : rows.data?.map((r) => (<tr key={r.id} className="hover:bg-gray-50"><td className="px-4 py-3 font-bold">{r.transaction_number}</td><td className="px-4 py-3">{r.date}</td><td className="px-4 py-3">{r.customer_name}</td><td className="px-4 py-3 text-gray-500">{r.operator}</td><td className="px-4 py-3 text-right font-bold">{formatRupiah(r.grand_total)}</td></tr>))}</tbody></table></div>
