...
export default function ReportMingguan({ rows, filters }) {
    ...
    const reload = (e) => {
        ...
        router.get(route('jihans.reports.mingguan'), p, { preserveState: true });
    };
        ...
        <JihansLayout pageTitle="Laporan Mingguan">
            ...
            <div className="space-y-6">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <h2 className="text-xl font-bold tracking-tight text-gray-800 dark:text-white/90">Laporan Mingguan</h2>
                        <p className="text-sm text-gray-500 dark:text-gray-400">Rekap transaksi dan metode pembayaran mingguan</p>
                    </div>
                    <a
                        href={route('jihans.reports.pdf', 'mingguan') + `?date_from=${form.date_from}&date_to=${form.date_to}`}
                        target="_blank"
                    ...
                    <div className="custom-scrollbar overflow-x-auto">
                        <table className="w-full text-left text-sm">
                            <thead className="border-b border-gray-200 bg-gray-50 text-[10px] font-bold uppercase tracking-wider text-gray-500 dark:border-gray-800 dark:bg-white/[0.02] dark:text-gray-400">
                                <tr>
                                    <th className="px-6 py-4 font-semibold">Minggu</th>
                            ...
                            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                                {rows.data?.length === 0 ? (
                                    <EmptyState colSpan={7} icon="date_range" message="Tidak ada data." />
                                ) : (
                                    rows.data?.map((r, i) => (
                                        <tr key={i} className="transition-colors hover:bg-gray-50/50 dark:hover:bg-white/[0.01]">
                                            <td className="px-6 py-4 text-gray-800 dark:text-white/90 font-mono font-semibold">{r.tahun_minggu}</td>
                                            <td className="px-6 py-4 text-xs text-gray-600 dark:text-gray-400 font-medium">{r.minggu_mulai} — {r.minggu_akhir}</td>
