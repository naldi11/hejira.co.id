...
export default function ReportPelanggan({ rows, filters }) {
    ...
    const reload = (e) => {
        ...
        router.get(route('jihans.reports.pelanggan'), p, { preserveState: true });
    };
        ...
        <JihansLayout pageTitle="Statistik Pelanggan">
            ...
            <div className="space-y-6">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <h2 className="text-xl font-bold tracking-tight text-gray-800 dark:text-white/90">Statistik Pelanggan</h2>
                        <p className="text-sm text-gray-500 dark:text-gray-400">Analisis pembelian, rentang tanggal transaksi, dan total belanja pelanggan</p>
                    </div>
                    <a
                        href={route('jihans.reports.pdf', 'pelanggan') + `?date_from=${form.date_from}&date_to=${form.date_to}&search=${form.search}`}
                        target="_blank"
                        ...
                        <table className="w-full text-left text-sm">
                            <thead className="border-b border-gray-200 bg-gray-50 text-[10px] font-bold uppercase tracking-wider text-gray-500 dark:border-gray-800 dark:bg-white/[0.02] dark:text-gray-400">
                                <tr>
                                    ...
                                    <th className="px-6 py-4 font-semibold">Pertama</th>
                                    <th className="px-6 py-4 font-semibold">Terakhir</th>
                            ...
                            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                                {rows.data?.length === 0 ? (
                                    <EmptyState colSpan={7} icon="group" message="Tidak ada data." />
                                ) : (
                                    rows.data?.map((r, i) => (
                                        <tr key={i} className="transition-colors hover:bg-gray-50/50 dark:hover:bg-white/[0.01]">
                                            <td className="px-6 py-4 font-bold text-gray-800 dark:text-white/90">{r.pelanggan}</td>
                                            <td className="px-6 py-4 text-gray-600 dark:text-gray-300">{r.tanggal_pertama}</td>
                                            <td className="px-6 py-4 text-gray-600 dark:text-gray-300">{r.tanggal_terakhir}</td>
