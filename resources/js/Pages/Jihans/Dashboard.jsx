...
const STAT_CARDS = [
    { key: 'produksi_hari_ini', label: 'Produksi Hari Ini', icon: 'factory', color: 'bg-orange-100 text-orange-600', suffix: 'batch' },
    ...
    return (
        <JihansLayout pageTitle="Dashboard Jihan's Food">
            ...
            <div className="mb-6 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
                {STAT_CARDS.map((c) => (
                    <div key={c.key} className="flex items-center gap-4 rounded-xl border border-orange-100 bg-white p-6 shadow-sm transition-shadow hover:shadow-md">
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
                    <div className="flex items-center justify-between border-b border-gray-100 p-5">
                        ...
                        <Link href={route('jihans.pos.index')} className="text-sm font-medium text-orange-600 hover:text-orange-700">Ke Kasir →</Link>
                    </div>
                            ...
                            {recentTransactions.map((trx) => (
                                <div key={trx.id} className="flex items-center justify-between p-4 hover:bg-gray-50">
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-500">{formatDate(trx.date)} • {trx.customer_name}</p>
                                    </div>
                ...
                <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
                    <div className="flex items-center justify-between border-b border-gray-100 p-5">
                        <h3 className="font-semibold text-gray-800">Stok Menipis (Butuh Request)</h3>
                        <Link href={route('jihans.stock.index')} className="text-sm font-medium text-orange-600 hover:text-orange-700">Lihat Stok →</Link>
                    </div>
                    {lowStocks.length === 0 ? (
                        <div className="p-5 text-center text-sm text-gray-500">Semua stok aman</div>
                    ) : (
                        <div className="divide-y divide-gray-100">
                            {lowStocks.map((st) => (
                                <div key={st.id} className="flex items-center justify-between p-4 hover:bg-gray-50">
                                    <div className="flex items-center gap-3">
                                        <div className="flex h-10 w-10 items-center justify-center rounded-lg border border-orange-100 bg-orange-50"><Icon name="inventory_2" className="text-[20px] text-orange-500" /></div>
                                        <div>
                                            <p className="font-medium text-gray-800">{st.name}</p>
                                            <p className="text-xs capitalize text-gray-500">{st.code} • {(st.jenis ?? '').replace('_', ' ')}</p>
                                        </div>
                                    </div>
