...
<head>
    ...
    <style>
        @page {
            ...
            ;
            margin-top:
                {{ $isDetailed ? '0.8cm' : '1.2cm' }}
            ;
            margin-bottom:
                {{ $isDetailed ? '1.0cm' : '0.6cm' }}
            ;
            margin-left: 0.6cm;
            margin-right: 0.6cm;
        }
...
<body>
    ...
    @else
        <div class="page-footer">
            <div class="page-footer-left">{{ now()->translatedFormat('d/m/Y H:i') }} &nbsp; &nbsp;
        ...
        <tr>
            ...
            <td class="period-cell">
                <div class="period-label">
                    ...
                    {{ $request->date_to ? \Carbon\Carbon::parse($request->date_to)->format('d/m/y') : \Carbon\Carbon::now()->format('d/m/y') }}
                </div>
            </td>
        </tr>
    </table>
            ...
            @foreach($rows as $txIndex => $tx)
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="padding: 4px 0; font-size: 8px; vertical-align: top;">
                            {{ \Carbon\Carbon::parse($tx->date)->format('d/m/Y') }}</td>
                        <td style="padding: 4px 0; font-size: 8px; vertical-align: top;">{{ strtoupper($tx->operator ?? '-') }}
                        </td>
                        <td style="padding: 4px 0; font-size: 8px; vertical-align: top;">{{ strtoupper($tx->customer_code) }}
                        </td>
                        <td style="padding: 4px 0; font-weight: bold; font-size: 8px; vertical-align: top;">
                            {{ strtoupper($tx->customer_name) }}</td>
                        <td style="padding: 4px 0; font-size: 8px; vertical-align: top;">{{ strtoupper($tx->customer_address) }}
                        </td>
                    </tr>
                                ...
                                <tbody>
                                    @foreach($tx->details as $index => $item)
                                        <tr>
                                            ...
                                            <td style="padding: 3px 0; text-align: right;">
                                                {{ $item->pot > 0 ? number_format($item->pot, 0, ',', '.') : '0' }}</td>
                                    ...
                                    <tr style="border-top: 1px dashed #000; border-bottom: 1px dashed #000; font-weight: bold;">
                                        ...
                                        <td style="padding: 3px 0; text-align: center;">
                                            {{ number_format($tx->details->sum('quantity'), 0, ',', '.') }}</td>
                                        <td style="padding: 3px 0;"></td>
                                        <td colspan="2" style="padding: 3px 0;"></td>
                                        <td style="padding: 3px 0; text-align: right;">
                                            {{ number_format($tx->details->sum('total'), 0, ',', '.') }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </td>
                    ...
                    <tr>
                        <td colspan="6" style="padding: 4px 0 12px 0; border-bottom: 1px dashed #000;">
                            <table
                                style="width: 95%; border-collapse: collapse; margin-left: 20px; font-size: 7.5px; font-weight: bold; border-top: 1px dashed #000; border-bottom: 1px dashed #000;">
                                <tr>
                                    <td style="width: 25%; text-align: left; padding: 4px 0;">Pot. :
                                        {{ number_format($tx->discount_total ?? 0, 0, ',', '.') }}</td>
                                    <td style="width: 25%; text-align: left; padding: 4px 0;">Pajak :
                                        {{ number_format($tx->tax_total ?? 0, 0, ',', '.') }}</td>
                                    <td style="width: 25%; text-align: left; padding: 4px 0;">Biaya : 0</td>
                                    <td style="width: 25%; text-align: right; padding: 4px 0;">Total Akhir :
                                        {{ number_format($tx->grand_total, 0, ',', '.') }}</td>
                                </tr>
                            </table>
                        </td>
                    </tr>
                ...
                @foreach($rows as $row)
                    <tr>
                        <td>
                            ...
                            @elseif($type === 'mingguan') {{ \Carbon\Carbon::parse($row->minggu_mulai)->format('d/m/Y') }} -
                                {{ \Carbon\Carbon::parse($row->minggu_akhir)->format('d/m/Y') }}
