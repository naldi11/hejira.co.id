...
<div class="max-w-4xl mx-auto" x-data="returnForm()">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <form action="{{ route('jihans.returns-to-gudang.store') }}" method="POST">
            @csrf
            
            <div class="p-6 border-b border-gray-100 bg-[#fff7ed]">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="date" name="date" value="{{ old('date', date('Y-m-d')) }}" required
                               class="w-full border-gray-300 rounded-lg focus:ring-[#f97316] focus:border-[#f97316] bg-white">
            ...
            <div class="p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <button type="button" @click="addItem" class="text-sm bg-orange-50 text-[#f97316] hover:bg-orange-100 px-3 py-1.5 rounded-lg font-medium transition-colors flex items-center gap-1">
                                ...
                                <tr class="border-b border-gray-100 last:border-0">
                                    <td class="py-3 px-2">
                                        <select :name="`items[${index}][product_id]`" x-model="item.product_id" required
                                                class="w-full text-sm border-gray-300 rounded-lg focus:ring-[#f97316] focus:border-[#f97316]">
            ...
            <div class="p-6 border-t border-gray-100 bg-orange-50 flex justify-between items-center gap-3">
                <p class="text-sm text-orange-700 font-medium">Stok Jihans akan otomatis dikurangi setelah return ini disubmit.</p>
                <div class="flex gap-2">
                    <a href="{{ route('jihans.returns-to-gudang.index') }}" class="px-5 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">Batal</a>
                    <button type="submit" class="px-5 py-2.5 text-sm font-medium text-white bg-[#f97316] hover:bg-[#ea580c] rounded-lg transition-colors shadow-sm">
