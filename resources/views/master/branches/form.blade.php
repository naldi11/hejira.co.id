                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        ...
                        <input type="text" name="code" value="{{ old('code', $branch->code ?? '') }}" required placeholder="cth: HND-CB3"
                               class="w-full px-5 py-3.5 bg-slate-50 border-2 border-slate-100 rounded-2xl text-sm font-bold text-slate-700 focus:bg-white focus:border-{{ $accentColor }}-500 focus:ring-4 focus:ring-{{ $accentColor }}-500/10 transition-all outline-none @error('code') border-rose-200 bg-rose-50 @enderror">
                    ...
                    <div class="md:col-span-2 space-y-2">
                        ...
                        <input type="text" name="name" value="{{ old('name', $branch->name ?? '') }}" required placeholder="cth: Hendhys Brownies SM Raja"
                               class="w-full px-5 py-3.5 bg-slate-50 border-2 border-slate-100 rounded-2xl text-sm font-bold text-slate-700 focus:bg-white focus:border-{{ $accentColor }}-500 transition-all outline-none @error('name') border-rose-200 bg-rose-50 @enderror">
                    ...
                    <div class="md:col-span-2 space-y-2">
                        ...
                        <textarea name="address" rows="3" placeholder="Jl. Raya No. 123..."
                                  class="w-full px-5 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl text-sm font-medium text-slate-800 placeholder:text-slate-400 focus:bg-white focus:border-{{ $accentColor }}-500 transition-all outline-none resize-none">{{ old('address', $branch->address ?? '') }}</textarea>
                    </div>
                </div>

                <div class="pt-4 flex flex-col sm:flex-row gap-3">
                    <button type="submit" class="flex-1 px-8 py-4 bg-slate-900 text-white rounded-2xl text-sm font-black uppercase tracking-widest hover:bg-{{ $accentColor }}-600 transition-all shadow-xl shadow-slate-900/10 active:scale-[0.98]">
        ...
        <div class="space-y-6">
            <div class="bg-{{ $accentColor }}-600 rounded-[2rem] p-8 text-white shadow-xl shadow-{{ $accentColor }}-600/20 relative overflow-hidden group">
                <div class="relative z-10">
                    ...
                    <ul class="space-y-4">
                        <li class="flex gap-3">
                            <span class="text-{{ $accentColor }}-300 text-xs">●</span>
