                            ...
                            <tr class="border-b border-surface-container hover:bg-surface-container-lowest/80 transition-colors group"
                                ...
                                <td class="px-md py-sm">
                                    <div class="flex flex-wrap gap-xs">
                                        @if($unit->visible_gudang)  <span class="inline-flex items-center gap-xs px-xs py-[2px] rounded-full font-label-sm text-label-sm bg-secondary-fixed text-on-secondary-fixed-variant border border-secondary-fixed-dim text-[11px]"><span class="material-symbols-outlined text-[12px]">warehouse</span>Gudang</span> @endif
                                        @if($unit->visible_jihans)  <span class="inline-flex items-center gap-xs px-xs py-[2px] rounded-full font-label-sm text-label-sm bg-secondary-fixed text-on-secondary-fixed-variant border border-secondary-fixed-dim text-[11px]"><span class="material-symbols-outlined text-[12px]">storefront</span>Jihan's</span> @endif
                                        @if($unit->visible_hendhys) <span class="inline-flex items-center gap-xs px-xs py-[2px] rounded-full font-label-sm text-label-sm bg-secondary-fixed text-on-secondary-fixed-variant border border-secondary-fixed-dim text-[11px]"><span class="material-symbols-outlined text-[12px]">cake</span>Hendhys</span> @endif
                                        @if(!$unit->visible_gudang && !$unit->visible_jihans && !$unit->visible_hendhys) <span class="inline-flex items-center px-xs py-[2px] rounded-full font-label-sm text-label-sm bg-surface-container text-on-surface-variant border border-outline-variant text-[11px]">—</span> @endif
                                    </div>
                                </td>
    ...
    <div id="modal-add"
        ...
        <div class="bg-surface-container-lowest rounded-xl shadow-2xl p-lg w-full max-w-sm mx-md">
            ...
            <form method="POST" action="{{ route(($routePrefix ?? 'master.') . 'units.store') }}" class="space-y-md">
                @csrf
                <div>
                    <label class="block font-label-sm text-label-sm text-on-surface-variant mb-xs">Nama Satuan <span class="text-error">*</span></label>
                ...
                <div>
                    <label class="block font-label-sm text-label-sm text-on-surface-variant mb-xs">Singkatan <span class="text-error">*</span></label>
                    <div class="bg-surface-container-low rounded-t-lg border-b border-outline-variant focus-within:border-primary focus-within:border-b-2 transition-all">
                ...
                <div class="flex gap-sm pt-xs">
                    <button type="submit" class="flex-1 bg-primary text-on-primary py-sm rounded-lg font-label-lg text-label-lg hover:bg-on-primary-fixed-variant transition-all">Simpan</button>
