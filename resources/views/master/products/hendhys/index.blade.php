...
@section('content')
    <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div class="flex flex-wrap items-center gap-3">
            ...
            <a href="{{ route(($routePrefix ?? 'master.') . 'products.create') }}"
                class="inline-flex items-center gap-2 px-6 py-3 bg-amber-600 text-white rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-amber-700 transition-all shadow-lg shadow-amber-600/20">
        ...
        <form method="GET" class="flex flex-wrap gap-4">
            <div class="flex-1 min-w-[280px] relative">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama atau kode produk..."
                    class="w-full pl-12 pr-4 py-3 bg-slate-50 border-2 border-slate-50 rounded-2xl focus:bg-white focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 transition-all outline-none text-sm">
            </div>
            
            <div class="min-w-[180px]">
                <select name="visibility"
                    class="w-full px-4 py-3 bg-slate-50 border-2 border-slate-50 rounded-2xl focus:bg-white focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 transition-all outline-none text-sm appearance-none cursor-pointer">
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors group">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-4">
                                    <div class="w-12 h-12 rounded-2xl overflow-hidden bg-slate-100 flex-shrink-0 flex items-center justify-center border border-slate-200 group-hover:border-amber-200 transition-colors">
                                        @if($product->image)
                            ...
                            <td class="px-6 py-4 text-right">
                                <span class="text-sm font-black text-amber-600">Rp{{ number_format($product->selling_price, 0, ',', '.') }}</span>
                            </td>
    ...
    <div x-data="{
        ...
        <div x-show="open"
            ...
            <div class="p-8 bg-[#6c2f00] text-white shrink-0">
                <div class="flex items-center gap-6">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-3 mb-2">
                            <span class="px-3 py-1 bg-amber-500 text-[10px] font-black uppercase tracking-widest rounded-lg" x-text="product.code"></span>
                        ...
                        <p class="text-amber-200 text-xs font-bold uppercase tracking-[0.2em] mt-1" x-text="product.category"></p>
                    </div>
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="flex items-center justify-between p-3 bg-amber-50 rounded-2xl border border-amber-100 text-amber-600">
                                ...
                                <tbody class="divide-y divide-slate-200">
                                    <template x-for="tier in product.tiers" :key="tier.min_qty">
                                        <tr>
                                            ...
                                            <td class="px-5 py-3 text-right text-sm font-black text-amber-600" x-text="'Rp' + fmt(tier.price)"></td>
                                        </tr>
                                    </template>
                                </tbody>
                            </table>
    ...
    <div id="importModal" class="hidden fixed inset-0 z-[100] flex items-center justify-center p-4" x-cloak>
        ...
        <div class="relative bg-white rounded-[2.5rem] shadow-2xl w-full max-w-lg overflow-hidden">
            <div class="p-8">
                <div class="w-16 h-16 bg-amber-50 text-amber-600 rounded-3xl flex items-center justify-center mb-6">
                ...
                <form action="{{ route(($routePrefix ?? 'master.') . 'products.import') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="flex gap-3">
                        ...
                        <button type="submit" 
                            class="flex-1 px-6 py-4 bg-amber-600 text-white rounded-2xl font-bold text-xs uppercase tracking-widest hover:bg-amber-700 transition-all shadow-lg shadow-amber-600/20">Import Data</button>
                    </div>
                </form>
            </div>
        </div>
