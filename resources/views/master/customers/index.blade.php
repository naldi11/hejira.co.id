...
<div class="flex items-center justify-between mt-4 mb-5">
    <div>
        ...
        <p class="text-sm text-gray-400">{{ $customers->total() }} data</p>
    </div>
...
<form method="GET" class="flex gap-2 mb-4">
    ...
    <select name="type" class="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none">
        ...
        <option value="agen" {{ request('type') === 'agen' ? 'selected' : '' }}>Agen</option>
    </select>
    ...
    <a href="{{ route('master.customers.index') }}" class="bg-gray-100 hover:bg-gray-200 border border-gray-300 text-gray-500 text-sm px-3 py-2 rounded-lg">Reset</a>
    @endif
</form>
    ...
    <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
            ...
            <tr class="hover:bg-gray-50">
                <td class="px-4 py-3 font-mono text-xs text-gray-500">{{ $customer->code }}</td>
                <td class="px-4 py-3 font-medium text-gray-800">{{ $customer->name }}</td>
                <td class="px-4 py-3">
                    <span class="inline-flex px-2 py-0.5 rounded-full text-xs font-medium {{ $customer->type === 'agen' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600' }}">
                        {{ ucfirst($customer->type) }}
                    </span>
                </td>
                <td class="px-4 py-3 text-gray-500">{{ $customer->phone ?? '-' }}</td>
                <td class="px-4 py-3 text-center">
                    <span class="inline-flex px-2 py-0.5 rounded-full text-xs font-medium {{ $customer->is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500' }}">
                        {{ $customer->is_active ? 'Aktif' : 'Nonaktif' }}
                    </span>
                </td>
                <td class="px-4 py-3 text-right">
                    <div class="flex items-center justify-end gap-2">
                        <a href="{{ route('master.customers.edit', $customer) }}" class="text-indigo-600 hover:text-indigo-800 text-xs font-medium">Edit</a>
            ...
            @empty
            <tr><td colspan="6" class="px-4 py-8 text-center text-gray-400">Tidak ada data customer.</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
