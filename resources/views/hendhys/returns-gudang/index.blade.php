...
<div class="bg-white rounded-xl shadow-sm border border-gray-200">
    <div class="p-6 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <form action="{{ route('hendhys.returns-to-gudang.index') }}" method="GET" class="flex flex-wrap items-center gap-3 w-full sm:w-auto">
            ...
            @if(request()->anyFilled(['status', 'search']))
                <a href="{{ route('hendhys.returns-to-gudang.index') }}" class="text-sm text-red-500 hover:text-red-700">Reset</a>
            @endif
        </form>
        <a href="{{ route('hendhys.returns-to-gudang.create') }}" class="bg-[#d97706] hover:bg-[#b45309] text-white px-5 py-2 rounded-lg transition-colors text-sm font-medium whitespace-nowrap flex items-center gap-2">
            ...
            <tbody class="divide-y divide-gray-100 text-sm">
                ...
                <tr class="hover:bg-amber-50/50 transition-colors">
                    ...
                    <td class="p-4 text-right">
                        <a href="{{ route('hendhys.returns-to-gudang.show', $ret->id) }}" class="text-[#d97706] hover:text-[#b45309] font-medium text-xs bg-amber-50 hover:bg-amber-100 px-3 py-1.5 rounded-md transition-colors inline-block">Detail</a>
                    </td>
                </tr>
                @empty
                <tr>
