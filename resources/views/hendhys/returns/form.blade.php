...
@section('page-title', 'Form Pengembalian Barang Cacat')

@section('content')
<div class="max-w-4xl mx-auto" x-data="returnForm()">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <form action="{{ route('hendhys.returns.store') }}" method="POST">
            @csrf
                    ...
                    <table class="w-full text-left">
                        <thead>
                            <tr class="bg-gray-50 text-gray-500 text-xs uppercase tracking-wider border-b border-gray-200">
                                <th class="pb-3 pt-2 px-2 font-medium">Produk Jadi</th>
                                ...
                                <tr class="border-b border-gray-100 last:border-0">
                                    ...
                                    <td class="py-3 px-2">
                                        <select :name="`items[${index}][condition]`" x-model="item.condition" required
                                            ...
                                            <option value="basi">Basi/Expired</option>
            ...
            <div class="p-6 border-t border-gray-100 bg-amber-50 flex justify-between items-center gap-3">
                <p class="text-sm text-amber-700 font-medium">Stok cabang akan otomatis berkurang saat return disubmit.</p>
                <div class="flex gap-2">
                    <a href="{{ route('hendhys.returns.index') }}" class="px-5 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">Batal</a>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('returnForm', () => ({
        items: [{ id: Date.now(), product_id: '', condition: 'rusak', qty: '', unit_id: '' }],
        
        addItem() {
            this.items.push({ id: Date.now(), product_id: '', condition: 'rusak', qty: '', unit_id: '' });
        },
