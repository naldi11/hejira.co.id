...
<div class="p-margin-mobile md:p-margin-desktop space-y-md overflow-y-auto custom-scrollbar h-full">
    ...
    <div class="bg-surface-container-lowest rounded-xl shadow-sm border border-outline-variant p-md">
        <form action="{{ route('hendhys.reports.bulanan') }}" method="GET" class="flex flex-wrap items-end gap-sm">
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('hendhys.reports.pdf', ['type' => 'bulanan'] + request()->all()) }}" target="_blank" class="h-10 px-md bg-error text-on-error rounded-lg font-label-lg uppercase tracking-wider hover:bg-red-700 transition-all shadow-sm flex items-center gap-2">
                    <span class="material-symbols-outlined text-[18px]">picture_as_pdf</span>
                    PDF
                </a>
                <a href="{{ route('hendhys.reports.bulanan') }}" class="h-10 px-md bg-surface-container-high text-on-surface rounded-lg font-label-lg uppercase tracking-wider hover:bg-surface-container-highest transition-all flex items-center">
                    Reset
                </a>
            </div>
        </form>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-surface-container-high border-b border-outline-variant">
                        <th class="px-md py-sm text-label-sm font-bold text-on-surface uppercase tracking-wider">Bulan</th>
