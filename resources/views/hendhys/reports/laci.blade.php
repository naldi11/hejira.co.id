...
@section('title', 'Laporan Laci Kasir')
@section('page-title', 'Laci Kasir: ' . auth()->user()->name)
...
<div class="p-margin-mobile md:p-margin-desktop space-y-md overflow-y-auto custom-scrollbar h-full">
    ...
    <div class="bg-surface-container-lowest rounded-xl shadow-sm border border-outline-variant p-md">
        <form action="{{ route('hendhys.reports.laci') }}" method="GET" class="flex flex-wrap items-end gap-sm">
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('hendhys.reports.pdf', ['type' => 'laci'] + request()->all()) }}" target="_blank" class="h-10 px-md bg-error text-on-error rounded-lg font-label-lg uppercase tracking-wider hover:bg-red-700 transition-all shadow-sm flex items-center gap-2">
                    <span class="material-symbols-outlined text-[18px]">picture_as_pdf</span>
                    PDF
                </a>
                <a href="{{ route('hendhys.reports.laci') }}" class="h-10 px-md bg-surface-container-high text-on-surface rounded-lg font-label-lg uppercase tracking-wider hover:bg-surface-container-highest transition-all flex items-center">
                    Reset
                </a>
            </div>
        </form>
